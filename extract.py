...
outDir_path = Path('data', img_name.replace('.bmp', '_outputs'))
...
dot_info = pd.DataFrame(
    np.stack(dot_info), 
    columns=['dot number', 'y-coord', 'x-coord'], 
    )

dot_info.to_csv(Path(outDir_path) / 'dot_info.csv', index=False)

np.savetxt(
    Path(outDir_path) / 'cmap.csv',
    cmap_img, delimiter=',', fmt='%d'
    )

np.savetxt(
    Path(outDir_path) / 'cmap-raw.csv',
...
plt.savefig(Path(outDir_path) / 'plot.png', dpi=dpi)
